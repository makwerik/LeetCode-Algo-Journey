...
### Пример задачи: [Remove Duplicates from Sorted List](https://github.com/makwerik/LeetCode-Algo-Journey/blob/master/remove_duplicates_from_sorted_list.py)
## Логика решения
Используя односвязный список, получаем голову списка. 
Дальше запускаем цикл пока данные в списке есть и пока ссылка на следующее значение есть.
Получаем первое значение, через val и сравниваем его со следующим next.val, если совпадают, то дубликат.
В таком случае заменяем ссылку на дубликат следующим значением, перепрыгивая дубликат
Иначе просто перемещаем курсор дальше.
![Remove Duplicates from Sorted List](result/Remove%20Duplicates%20from%20Sorted%20List.png)

### Пример задачи: [Merge Sorted Array](https://github.com/makwerik/LeetCode-Algo-Journey/blob/master/merge_sorted_array.py)
## Логика решения
Нужно объединить список nums1 и nums2 в один nums1 с сортировкой по возрастанию.
Для этого мы создаём несколько указателей в функции, один указывает последний элемент в nums1, другой
на последний элемент nums 2, и третий указываем на общую длину списка.
После запускаем цикл пока второй указатель не закончится и сравниваем между собой последние числа, самое больше помещаем в конец списка nums1
...
### Пример задачи: [Same Tree](https://github.com/makwerik/LeetCode-Algo-Journey/blob/master/same_tree.py)
## Логика решения
Проверка того, что перед нами одно и то же дерево.
Сначала проверяем узлы, если оба пусты - деревья одинаковы.
Если одно из них пусто - деревья разные.
Если значения в узлах разные - деревья разные.
Иначе возвращаем рекурсию на левое и правое поддерево через логическое "И"
![Same Tree](result/Same%20Tree.png)

### Пример задачи: [Symmetric Tree](https://github.com/makwerik/LeetCode-Algo-Journey/blob/master/symmetric_tree.py)
## Логика решения
Проверка зеркальности деревьев.
Сначала проверяем пустой ли корень, если да - значит возвращаем истину.
Если нет - переходим в рекурсивную функцию.
Запускаем череду проверок: пустые узлы - деревья одинаковые, если один из узлов пуст - деревья разные, если значения
левого и правого поддерева не совпадают - деревья разные.
И возвращаем результат рекурсии, чтобы его не потерять через логическое "И", где мы сначала внешние узлы (левый ребёнок левого дерева и правый ребёнок правого дерева)
и затем внутренние узлы (правый ребёнок левого дерева и левый ребёнок правого дерева)
![Symmetric Tree](result/Symmetric%20Tree.png)

### Пример задачи: [Maximum Depth of Binary Tree](https://github.com/makwerik/LeetCode-Algo-Journey/blob/master/maximum_depth_of_binary_tree.py)
## Логика решения
Нужно посчитать глубину дерева.
Сначала проверяем корень на пустоту, если он пуст - значит глубина = 0.
Потом используем формулу для подсчёта глубины сначала левого, затем правого поддерева.
Глубину получаем тогда, когда рекурсия доходит до листьев (где возвращается 0), а затем при подъёме вверх
а каждом уровне добавляется +1 ... , формируя итоговую глубину.
![Maximum Depth of Binary Tree](result/Maximum%20Depth%20of%20Binary%20Tree.png)


### Пример задачи: [Convert Sorted Array to Binary Search Tree](https://github.com/makwerik/LeetCode-Algo-Journey/blob/master/convert_sorted_array_to_binary_search_tree.py)
## Логика решения
Нужно построить дерево из отсортированного массива.
1. Сначала проверяем что массив не пуст.
2. Дальше находим середину массива.
3. После создаём корень дерева со значением из центра массива
4. Вызываем рекурсию для левого поддерева забирая срезом все значения слева.(пункт 2.3 в рекурсии повторяется)
5. То же самое для правого поддерева.(пункт 2.3 в рекурсии повторяется)
![Convert Sorted Array to Binary Search Tree](result/Convert%20Sorted%20Array%20to%20Binary%20Search%20Tree.png)
